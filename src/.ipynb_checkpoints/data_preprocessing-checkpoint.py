...
def load_data(path = '', file_name = 'kaufpreissammlung-liegenschaften.csv'):
...
def export_data_to_parquet(df, output_dir):
    ...
    try:
        ...
        file_path = os.path.join(export_dir, 'output_data.parquet')
...
def main_data_preprocessing():
    df = load_data(path = '/home/daniel/Dokumente/FH_StPölten/Clean_Coding/CCSE_Repro/data', file_name = 'kaufpreissammlung-liegenschaften.csv')
    df = select_cols(df)
